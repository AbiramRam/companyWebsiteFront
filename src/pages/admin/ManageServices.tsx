...
export default function ManageServices() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    await fetch('https://charismatic-art-production-3e7a.up.railway.app/api/services', {
      method: 'POST',
      headers: {
  ...
  const handleDelete = async (id: number) => {
    const token = localStorage.getItem('adminToken');
    await fetch(`https://charismatic-art-production-3e7a.up.railway.app/api/services/${id}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${token}`
      }
